...
function scanMOGRTClips(schemaPath) {
    try {
        var seq = app.project.activeSequence;
        if (!seq) return JSON.stringify({ error: "No hay secuencia activa" });

        // Load schemas from temp file (generated by client from definition.json)
        var schemas = {};
        if (schemaPath) {
            try {
                var sf = new File(schemaPath);
                sf.open("r");
                var sc = sf.read();
                sf.close();
                schemas = JSON.parse(sc);
            } catch(eSchema) {
                $.writeln("[scanMOGRTClips] Error loading schemas: " + eSchema.message);
            }
        }
        ...
        for (var t = 0; t < seq.videoTracks.numTracks; t++) {
            ...
            for (var c = 0; c < track.clips.numItems; c++) {
                ...
                // Extract type tag from clip name: [BULLET] → BULLET
                var typeTag = "";
                var tm = clipName.match(/^\[([A-Z]+)\]/);
                if (tm) typeTag = tm[1];

                // Get schema for this MOGRT type
                var schema = schemas[typeTag] || null;

                // Read properties using schema (or discover all if no schema)
                var props = [];
                for (var p = 0; p < moComp.properties.numItems; p++) {
                    ...
                    // Determine property type from schema or skip if unknown
                    var propDef = schema ? schema[dn] : null;
                    if (!propDef) {
                        // No schema entry — skip unknown properties
                        continue;
                    }

                    var info = { name: dn, index: p, type: propDef.type };
                    try {
                        var val = prop.getValue();

                        if (propDef.type === "color") {
                            // Try getColorValue first, fall back to getValue
                            var colorVal = null;
                            try { colorVal = prop.getColorValue(); } catch(ecv) {}
                            if (colorVal && typeof colorVal === "object") {
                                // getColorValue returns an object, extract RGB
                                var rr, gg, bb;
                                if (colorVal.length >= 3) {
                                    rr = Math.min(255, Math.max(0, Math.round(colorVal[0] * 255)));
                                    gg = Math.min(255, Math.max(0, Math.round(colorVal[1] * 255)));
                                    bb = Math.min(255, Math.max(0, Math.round(colorVal[2] * 255)));
                                } else {
                                    rr = 255; gg = 255; bb = 255;
                                }
                            } else if (typeof val === "object" && val !== null && val.length >= 3) {
                                rr = Math.min(255, Math.max(0, Math.round(val[0] * 255)));
                                gg = Math.min(255, Math.max(0, Math.round(val[1] * 255)));
                                bb = Math.min(255, Math.max(0, Math.round(val[2] * 255)));
                            } else {
                                rr = 255; gg = 255; bb = 255;
                            }
                        ...
                        } else if (propDef.type === "slider") {
                            info.value = (typeof val === "number") ? val : parseFloat(val) || 0;
                            info.min = propDef.min !== undefined ? propDef.min : 0;
                            info.max = propDef.max !== undefined ? propDef.max : 100;

                        } else if (propDef.type === "dropdown") {
                            info.value = (typeof val === "number") ? val : parseInt(val) || 0;
                            info.options = propDef.options || [];
