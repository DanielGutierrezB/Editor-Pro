  ...
  <Safe>
    ...
    <Anim type="fade-up" delay={8}>
      <div style={{fontSize:32,color:C.text,textAlign:'center',marginTop:24,maxWidth:900}}>
        Imagina que cada vez que avanzas en tu proyecto
      </div>
    </Anim>
    <Anim type="pop" delay={16}>
      <Camera size={80} color={C.accent} style={{marginTop:40}} />
    </Anim>
  </Safe>
);
  ...
  <Safe>
    ...
    <Anim type="pop" delay={16}>
      <div style={{display:'flex',gap:32,marginTop:48,alignItems:'center'}}>
        <Camera size={60} color={C.accent} />
        <Clock size={60} color={C.accent} />
      </div>
    </Anim>
    <Anim type="fade-up" delay={24}>
      <div style={{background:C.card,borderRadius:16,padding:'24px 48px',marginTop:32,fontSize:28,color:C.accent,fontWeight:700}}>
...
export const M01Reveal262605V1:React.FC = () => (
  <AbsoluteFill style={{backgroundColor:C.bg,fontFamily:"'DM Sans',sans-serif"}}>
    <Section from={0} dur={148}><Slide1/></Section>
    <Section from={148} dur={149}><Slide2/></Section>
  </AbsoluteFill>
);